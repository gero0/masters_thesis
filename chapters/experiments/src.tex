...
\begin{algorithm}[]
    ...
    $S \gets \{\}$\;
    $P \gets generateAllPermutations()$\;
    \ForEach{$p \in P$}{
        $lo \gets 2opt(p)$\;
        $S \gets S \cup \{(p, lo)\}$\;
    }
    ...
    \SetKwFunction{FI}{inrange\_2exchange}
    ...
    \Fn{\FI{$p, n, D$}}{
        ...
        \For{$i \in 1..D$}{
            ...
            \ForEach{$perm \in perms$}{
                $child\_perms \gets 2exchange\_allperms(perm)$\;
                \ForEach{$child \in child\_perms$}{
                    \If{child = n}{
                        \Return{true}\;
                    }
                    $new\_perms \gets new\_perms \cup \{child\}$\;
                }
            }
            $perms \gets new\_perms$\;
        }
        \Return{false}\;
    }
    \textbf{end}
...
TODO: opisać operację 2-exchange i operację kicka i generowania wszystkich możliwych kicków w przeglądzie zupełnym
